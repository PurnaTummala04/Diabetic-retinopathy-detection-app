...
def new_assessment():
    ...
    if request.method == "POST":
        ...
        if pisa_status.err:
            app.logger.error(f"PISA PDF Generation Error: {pisa_status.err}")
            flash("Assessment created but PDF generation failed. Contact admin.", "warning")
            return redirect(url_for("patient_dashboard"))
...
# ============================================
# ✅ NEW ROUTE FOR DOWNLOADING EXISTING PDFs
# ============================================
@app.route("/report/<report_id>/download")
@login_required
def download_report_pdf(report_id):
    # --- Check Permissions ---
    # Allow admins and doctors to download any report
    if not is_admin() and current_user.role != "doctor":
        # Check if the user is a patient trying to download their OWN report
        try:
    ...
    # --- Re-use the EXACT PDF generation logic from /assessment/new ---
    patient_id = report_doc.get("patient_id")
    doctor_id = report_doc.get("doctor_id")

    patient = users_col.find_one({"_id": patient_id}) if patient_id else {}
    doctor = users_col.find_one({"_id": doctor_id}) if doctor_id else {}
    ...
    context = {
        ...
        "doctor_name": (doctor or {}).get("name", "Not assigned"),
        "doctor_username": (doctor or {}).get("username", ""),
        "created_at": to_ist_str(report_doc.get("created_at")),
        "features": report_doc.get("features", {}),
        "probability": report_doc.get("probability", 0.0),
        "prediction": report_doc.get("prediction", 0),
        "threshold": report_doc.get("threshold", ENSEMBLE_THRESHOLD_CUSTOM),
        "ensemble_balanced_accuracy": report_doc.get("ensemble_balanced_accuracy", 0.0),
    ...
    if pisa_status.err:
        app.logger.error(f"PISA PDF Generation Error for report {report_id}: {pisa_status.err}")
        flash("Could not generate PDF for this report. The PDF template may have an error.", "danger")
...
def doctor_dashboard():
    ...
    cache = {}
    processed_reports = [] # Use a new list to hold valid reports
    for r in reports:
        pid = r.get("patient_id")
        if not pid:
            continue # Skip this broken report
        ...
        r["id_str"] = str(r["_id"])
        r["created_str"] = to_ist_str(r.get("created_at"))
        processed_reports.append(r) # Add the valid report to the list
    
    reports = processed_reports # Overwrite old list with the cleaned one
...
def doctor_bulk_download(bid):
    ...
    if not path or not os.path.exists(path):
        flash("Export file missing. It may have been deleted by a server restart. Please re-upload.", "danger")
